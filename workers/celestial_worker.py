"""
Celestial Worker für RabbitMQ
Berechnet Planeten, Sonne, Mond mit Skyfield
"""
import pika
import json
import time
...
from api.computation import LOADER, ts, eph, compute_celestial_bodies
...
class CelestialWorker:
    """
    RabbitMQ Worker für Celestial-Berechnungen (Planeten, Sonne, Mond)
    """
    
    def __init__(self, rabbitmq_url: str, worker_id: str = "celestial-worker-1"):
        """
        Initialisiert Celestial Worker
    ...
    def compute_celestial(self, location_dict, time_bucket_str):
        """
        Berechnet Celestial Bodies (Planeten, Sonne, Mond) mit Skyfield
        ...
        try:
            ...
            # Celestial Bodies berechnen
            celestial_data = compute_celestial_bodies(
                LOADER, ts, eph, location, time_bucket_dt
            )
            
            logger.info(f"Computed {len(celestial_data)} celestial bodies for {location_dict}")
            return celestial_data
            
        except Exception as e:
            logger.error(f"Error computing celestial bodies: {e}")
            raise
    ...
    def callback(self, ch, method, properties, body):
        ...
        try:
            ...
            # Berechnung
            results = self.compute_celestial(location, time_bucket)
...
if __name__ == '__main__':
    ...
    worker_id = os.environ.get('WORKER_ID', 'celestial-worker-1')
    
    # Worker starten
    worker = CelestialWorker(rabbitmq_url, worker_id)
