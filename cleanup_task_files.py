...
def analyze_task_files(dry_run=True, max_age_hours=24):
    ...
    print(f"Gefunden: {len(task_files)} Task-Dateien, {len(status_files)} Status-Dateien")
    ...
    for task_id in task_ids:
        ...
        print(f"\nTask {task_id}:")
        print(f"  Task-Datei: {'✓' if task_file else '✗'} (Alter: {task_age:.1f}h)")
        print(f"  Status-Datei: {'✓' if status_file else '✗'} (Alter: {status_age:.1f}h)")
        print(f"  Status: {status}")
        print(f"  Max-Alter: {max_age:.1f}h")
    ...
    print(f"Abgeschlossene alte Tasks: {len(completed_old_files)} Dateien")
    print(f"Fehler-Tasks: {len(error_old_files)} Dateien")
    print(f"Hängende Tasks: {len(running_old_files)} Dateien")
    print(f"Verwaiste Dateien: {len(orphaned_files)} Dateien")
    print(f"GESAMT zu löschende Dateien: {len(all_orphaned)}")
    
    if all_orphaned:
        print(f"\nDateien älter als {max_age_hours}h:")
        for filepath in all_orphaned:
            age = get_file_age_hours(filepath)
            print(f"  {filepath} (Alter: {age:.1f}h)")
        ...
        if not dry_run:
            ...
            for filepath in all_orphaned:
                try:
                    ...
                    print(f"  ✗ Fehler beim Löschen {filepath}: {e}")
            print(f"Erfolgreich gelöscht: {deleted}/{len(all_orphaned)} Dateien")
        else:
            print(f"\nDRY-RUN: Würde {len(all_orphaned)} Dateien löschen")
            print("Führe mit --delete aus, um tatsächlich zu löschen")
...
def main():
    ...
    if "--delete" in sys.argv:
        dry_run = False
        print("WARNUNG: Lösche tatsächlich Dateien!")
    ...
    if "--max-age" in sys.argv:
        try:
            ...
            print("Fehler: --max-age benötigt eine Zahl")
            sys.exit(1)
    
    print("Task-Dateien Cleanup")
    print(f"Max-Alter: {max_age_hours} Stunden")
    print(f"Modus: {'LÖSCHEN' if not dry_run else 'DRY-RUN'}")
    print("="*60)
