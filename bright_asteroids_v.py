...
def load_bright_asteroids(loader, ts, eph, observer_location, max_magnitude=MAX_APPARENT_MAGNITUDE, use_cache=True, current_dt: Optional[datetime] = None):
    ...
    try:
        # Initial filtering from the main DataFrame
        ...
        orbits = [mpc.mpcorb_orbit(row, ts, gm_km3_s2=GM_SUN_Pitjeva_2005_km3_s2) for _, row in candidates_df.iterrows()]
        targets = [(sun + orbit) if int(getattr(orbit, 'center', 10)) != 0 else orbit for orbit in orbits]

        # 2. Hybrid observation (iteration required for Skyfield's observe)
        observations = [observer.at(t).observe(target) for target in targets]

        # 3. Vectorize remaining calculations
        ...
        G_values = candidates_df['magnitude_G'].to_numpy()
        
        apparent_magnitudes = vectorized_asteroid_apparent_magnitude(
            H=H_values, G=G_values, r=rs, delta=deltas, phase_angle_deg=phase_angles
        )
        
        candidates_df['apparent_magnitude'] = apparent_magnitudes

        # Filter by apparent magnitude and sort
        ...
        # --- Final Processing (remains iterative for rise/set times) ---
        asteroid_list = []
        events_computed = 0
        for index, row in top_df.iterrows():
            try:
                ...
                # Use existing observation data where possible
                # Note: Re-observing for apparent is cheap
                astrometric = observer.at(t).observe(target)
                ...
                if events_computed < ASTEROIDS_EVENTS_MAX:
                    # Logic for rise/set/transit remains the same
                    start_time = ts.utc(t.utc_datetime().replace(hour=0, minute=0, second=0, microsecond=0))
                    end_time = ts.utc(start_time.utc_datetime() + timedelta(days=2))
                    ...
                    if t_times:
                        # Find next transit time after current time
                        now_utc = dt_utc if dt_utc.tzinfo else dt_utc.replace(tzinfo=timezone.utc)
                        future_transits = [ti.utc_datetime() for ti in t_times if ti.utc_datetime() > now_utc]
                        if future_transits:
                            transit_time = min(future_transits)
                        else:
                            # Fallback to the first available transit if all are in the past
                            transit_time = t_times[0].utc_datetime()
